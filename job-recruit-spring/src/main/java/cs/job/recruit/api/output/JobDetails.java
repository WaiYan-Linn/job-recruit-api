...
public record JobDetails(Long id, String title, Category category, String location, JobType jobType, WorkMode workMode,
	...
	public static void select(CriteriaBuilder cb, CriteriaQuery<JobDetails> cq, Root<Job> root) {
		...
		cq.multiselect(root.get(Job_.id), root.get(Job_.title), // String
				...
				cb.construct(EmployerBasic.class, employerJoin.get(Employer_.id),
						employerJoin.get(Employer_.companyName), employerJoin.get(Employer_.website),
						employerJoin.get(Employer_.profilePictureUrl)), // Employer entity
				root.get(Job_.closed));
